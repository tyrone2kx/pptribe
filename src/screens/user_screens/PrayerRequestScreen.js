...
const PrayerRequestScreen = (props) => {
    ...
    const addPrayerRequest = () => {
        api.post('add_prayer_request', { title, description }).then(response => {
                ...
                <ScrollView alwaysBounceVertical style={{ minHeight: height, backgroundColor: colors.primaryBackground, }}>
                    ...
                    <View style={{ width: '90%', alignSelf: 'center', marginBottom: 30, marginTop: 20 }}>
                        <TouchableOpacity activeOpacity={0.5} style={{ ...styles.addTestimonyBtn, backgroundColor: openForm ? colors.red : colors.blue, }} onPress={() => setOpenForm(!openForm)}>
                            <Text style={{ color: '#FFF' }}>{openForm ? 'CANCEL' : 'SUBMIT A PRAYER REQUEST'}</Text>
                        </TouchableOpacity>
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(PrayerRequestScreen);
