...
const EditPrayerRequestScreen = (props) => {
    ...
    const fetchRequest = () => {
        api.post('fetch_single_request', { id }).then(response => {
            const ref = response.data;
            props.stopLoading()
            if (ref.success === true) {
                setDescription(ref.request.description)
                setTitle(ref.request.title)
            }
            else {
    ...
    const editPrayerRequest = () => {
        api.post('edit_prayer_request', { title, description, id }).then(response => {
            const ref = response.data;
            props.stopLoading()
            if (ref.success === true) {
...
export default connect(mapStateToProps, mapDispatchToProps)(EditPrayerRequestScreen);
